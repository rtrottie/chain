...
from ws_chains import load_optimized_species_no_U, load_optimized_U_species
...
class NaDiffusionPBEUChain(SpinCustomChain):
    def __init__(self, vaspobj: Vasp(), nupdowns, standard=[], override=[], **kwargs):
        ...
        get_nopsin_eig_gamma = CustomFunctional(Vasp, standard + [get_eigen_nospin, set_algo_fast] + gamma + override)
        get_eigenvalues= CustomFunctional(Vasp, standard + [get_eigen, set_algo_fast] + override)
        get_eigenvalues_gamma = CustomFunctional(Vasp, standard + [get_eigen, set_algo_fast] + gamma + override)
        final_converge = CustomFunctional(Vasp, standard + [full_converge, all_output] + override)
        final_converge_gamma = CustomFunctional(Vasp, standard + [full_converge] + gamma + override)
        sp = CustomFunctional(Vasp, standard + [full_converge, all_output] + override)
...
def diffusion_standard_na(vasp, structure=None):
    vasp.ediff = 1e-5
    vasp.encut = 520
    vasp.ismear = 0
    vasp.sigma = 0.1
